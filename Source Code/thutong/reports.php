...
if($showreport=='menu'){
	$PAGE->set_url(MOD_THUTONG_URL . '/reports.php', array('id' => $cm->id));
}else{
	$PAGE->set_url(MOD_THUTONG_URL . '/reports.php', 
...
if($CFG->version<2014051200){
	add_to_log($course->id, MOD_THUTONG_MODNAME, 'reports', "reports.php?id={$cm->id}", $moduleinstance->name, $cm->id);
}else{
...
$reportrenderer = $PAGE->get_renderer(MOD_THUTONG_FRANKY,'report');
...
switch ($showreport){
	...
	case 'menu':
		echo $renderer->header($moduleinstance, $cm, $mode, null, get_string('reports', MOD_THUTONG_LANG));
	...
	case 'basic':
		$report = new mod_thutong_basic_report();
